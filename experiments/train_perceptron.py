...
# =========================
# MODELO PERCEPTRÓN ONLINE — PREQUENTIAL
# Perceptrón no tiene predict_proba, se usa decision_function para la curva ROC
# class_weight no es compatible con partial_fit, se usa sample_weight por instancia
# =========================
...
for i in range(len(X_scaled)):
    ...
    if i > 0:
        ...
        y_score = model.decision_function(x)[0]
    ...
    model.partial_fit(
        ...
        [y_true],
        classes=classes,
        sample_weight=[class_weight_dict[y_true]]
    )
...
plt.xlabel("FPR")
plt.ylabel("TPR")
plt.title("ROC - Perceptrón Online")
plt.legend()
plt.grid()
