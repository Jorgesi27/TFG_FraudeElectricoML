...
with open("results/roc_naive_bayes_incremental.pkl", "wb") as f:
    pickle.dump({
        "fpr": fpr_curve,
        "tpr": tpr_curve,
        "auc": roc_auc
...
plt.figure()
plt.plot(fpr_curve, tpr_curve, label=f"Naive Bayes Incremental (AUC = {roc_auc:.3f})")
plt.plot([0, 1], [0, 1], linestyle='--')
plt.xlabel("False Positive Rate")
plt.ylabel("True Positive Rate (Recall)")
plt.title("Curva ROC - Naive Bayes Incremental")
plt.legend()
plt.grid()

plt.savefig("results/roc_naive_bayes_incremental.png", dpi=300)
plt.close()

print("\n===== Resultados Naive Bayes Incremental =====")
