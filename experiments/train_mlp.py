...
fold_df.to_csv("results/kfold_mlp.csv", index=False)
print("\nResultados k-fold guardados en results/kfold_mlp.csv")
...
with open("results/roc_mlp.pkl", "wb") as f:
    pickle.dump({
        "fpr": fpr_curve,
        "tpr": tpr_curve,
        "auc": roc_auc
...
plt.xlabel("FPR")
plt.ylabel("TPR")
plt.title("ROC - Perceptrón Multicapa")
plt.legend()
plt.grid()
plt.savefig("results/roc_mlp.png", dpi=300)
plt.close()
