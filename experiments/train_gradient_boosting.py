...
for fold, (train_idx, test_idx) in enumerate(tscv.split(X_all)):
    ...
    m.fit(X_tr, y_tr, sample_weight=sample_weights_fold)
...
fold_df.to_csv("results/kfold_gradient_boosting.csv", index=False)
print("\nResultados k-fold guardados en results/kfold_gradient_boosting.csv")
...
with open("results/roc_gradient_boosting.pkl", "wb") as f:
    pickle.dump({
        "fpr": fpr_curve,
        "tpr": tpr_curve,
        "auc": roc_auc
...
plt.legend()
plt.grid()
plt.savefig("results/roc_gradient_boosting.png", dpi=300)
plt.close()
...
importance.to_csv("results/gradient_boosting_feature_importance.csv", index=False)
