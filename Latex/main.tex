...
\lstdefinelanguage{SQL}{
    keywords={
        CREATE, DATABASE, TABLE, PRIMARY, KEY,
        AUTO_INCREMENT, NOT, NULL, UNIQUE,
        DEFAULT, CURRENT_TIMESTAMP, FOREIGN,
        REFERENCES, ENUM, INT, VARCHAR,
        FLOAT, TIMESTAMP, LONGTEXT, USE
    },
    sensitive=false
}
...
La digitalización de las redes eléctricas y las infraestructuras de medición inteligente han ido progresivamente transformando la gestión y recopilación de la información del consumo energético. Destacan los contadores inteligentes y las redes eléctricas inteligentes, \emph{smart grids}, que permiten generar grandes cantidades de datos de consumo de forma continua, detallada y prácticamente en tiempo real. Proporcionando una oportunidad muy importante para la aplicación de técnicas de aprendizaje automático capaces de analizar automáticamente patrones de comportamiento y detectar anomalías relacionadas a posibles fraudes eléctricos.

Como el fraude en el consumo de electricidad genera grandes pérdidas económicas y tiene un impacto negativo en la red eléctrica, es uno de los problemas más importantes para las empresas distribuidoras de energía. Las prácticas fraudulentas más comunes incluyen las conexiones ilegales a la red y las manipulaciones de contadores o instalaciones eléctricas para disminuir el consumo registrado de manera artificial. En España, de acuerdo con información divulgada por Endesa \cite{endesa}, se han identificado en los últimos años centenares de miles de casos de fraude eléctrico, lo que demuestra la urgencia de crear sistemas automáticos que sean más eficaces para su detección.

Tradicionalmente, se emplean métodos de aprendizaje supervisado en modo \emph{offline} para detectar fraude eléctrico. Los modelos son entrenados con conjuntos de datos históricos completos y se utilizan para clasificar consumos recientes. El interés por técnicas de aprendizaje \emph{online} que trabajan con información de forma secuencial y se ajustan a nuevos patrones de consumo ha aumentado debido al crecimiento de las infraestructuras inteligentes y a la disponibilidad de datos constantes. La posibilidad de detectar comportamientos no normales en tiempo real puede ser muy beneficiosa para acortar los tiempos de reacción ante acciones fraudulentas y optimizar la capacidad del sistema para adaptarse a modificaciones en el consumo.

Este proyecto propone una comparativa entre técnicas de aprendizaje automático \emph{offline} y técnicas \emph{online} aplicadas a la detección de fraude eléctrico. Además de evaluar el rendimiento de predicción de ambos, se desarrolla un prototipo funcional que permite simular un entorno operativo real, incorporando tanto predicción histórica como procesamiento secuencial de datos en tiempo real.


\section{Objetivo}

El propósito principal es diseñar e implementar un sistema de detección de fraude en el consumo eléctrico que emplee métodos de aprendizaje automático, aplicado a conjuntos de datos de los usuarios. Que permite comparar los modelos de aprendizaje \emph{offline} con los modelos de aprendizaje \emph{online}. Para alcanzar esta meta general se proponen algunos objetivos más específicos como la implementación de seis métodos relevantes de aprendizaje \emph{offline} y de cinco métodos importantes de aprendizaje \emph{online}. Después se analizan ambos grupos de modelos con métricas adecuadas para determinar la técnica más eficaz en cada categoría y, por último, añadir los modelos seleccionados a un prototipo de software que permita comparar visualmente los resultados. En el sistema desarrollado debe poder analizar el comportamiento de los modelos en un entorno controlado, facilitando la comparación objetiva entre modelos \emph{online} y \emph{offline}.


\section{Alcance}

El proyecto aborda el análisis, desarrollo e implementación de técnicas de aprendizaje automático aplicadas a la detección de fraude en el consumo eléctrico. Para ello, el trabajo se estructura en varias fases diferenciadas que abarcan desde el estudio del estado del arte hasta el desarrollo de un prototipo funcional. Empezando por un análisis de las técnicas actuales utilizadas en este dominio, estudiando las metodologías propuestas en artículos científicos y trabajos. En esta revisión se incluyen métodos basados en aprendizaje supervisado, métodos de detección de anomalías, técnicas estadísticas y algoritmos de aprendizaje \emph{online} y \emph{offline}. 

Después, se lleva a cabo un análisis de las fuentes de datos disponibles para el entrenamiento y evaluación de modelos de aprendizaje automático. Se estudian distintos conjuntos de datos públicos relacionados con el consumo eléctrico, incluyendo tanto datos etiquetados con usuarios fraudulentos o no fraudulentos como datos sin etiquetar orientados a técnicas no supervisadas. A continuación, se realiza el estudio, selección e implementación de diferentes modelos de aprendizaje automático. Los métodos implementados se clasifican en dos grupos principales: técnicas \emph{offline}, entrenadas a partir de datos históricos, y técnicas \emph{online}, que se adaptan continuamente a nuevos datos en tiempo real. Inicialmente se planteó la implementación de cinco técnicas para cada categoría, aunque finalmente se añadió un método offline adicional por su relevancia y rendimiento.

Seguidamente, se desarrolla una evaluación comparativa de las técnicas implementadas, analizando distintos rasgos como la capacidad de detección de fraude, la robustez frente a conjuntos de datos desbalanceados, la adaptabilidad ante cambios en los patrones de consumo y el coste computacional. Esta comparación permitirá identificar las técnicas con mejor rendimiento según el escenario de aplicación, diferenciando entre aprendizaje \emph{offline} y aprendizaje \emph{online}.

Como parte práctica del proyecto, se diseña e implementa un prototipo \emph{software} basado en un entorno web interactivo. Dicho sistema permitirá simular escenarios de detección de fraude eléctrico utilizando tanto datos históricos como datos en tiempo real. Además, el prototipo integrará los dos modelos con mejor rendimiento, mostrará los resultados obtenidos y facilitará la comparación visual entre los modelos \emph{offline} y \emph{online}. Finalmente, se presentan las conclusiones del proyecto, resumiendo los resultados obtenidos, las ventajas y limitaciones de cada metodología y posibles líneas de trabajo futuro orientadas a la mejora del sistema desarrollado.


\section{Estado del arte}

En esta sección se analizan los principales métodos existentes utilizados para detectar fraude en el consumo eléctrico. Se estudian tanto métodos más tradicionales y clásicos, como los métodos más recientes y avanzados. Además, se incluye un análisis amplio de los conjuntos de datos que existen en la literatura.
...
donde $l_t$ representa las pérdidas no técnicas (incluyendo robo de electricidad) y $\delta_t$ representa las pérdidas técnicas. La principal limitación de las técnicas de esta subcategoría es que no se puede localizar con precisión al usuario fraudulento. Además, este método no distingue entre pérdidas técnicas y no técnicas, por lo que resulta insuficiente en escenarios reales complejos. Por ello, se han desarrollado métodos más avanzados y orientados por datos.
...
Las tareas de aprendizaje supervisado se pueden dividir en problemas de clasificación y regresión. La clasificación, en los métodos de aprendizaje automático, utiliza un algoritmo para clasificar los datos en categorías, reconoce entidades específicas dentro del conjunto de datos e intenta determinar cómo se deben etiquetar o definir esas entidades. Algunos de los algoritmos más comunes son los clasificadores lineales, las máquinas de vectores de soporte (SVM), los árboles de decisión (DT), el k vecino más cercano (KNN), la regresión logística y el bosque aleatorio (RF). Los métodos ensemble son un metaenfoque del aprendizaje supervisado en el que se entrenan varios modelos en la misma tarea de clasificación o regresión. Los resultados de todos los modelos del grupo se agregan para descubrir la mejor estrategia general para resolver el desafío.

Los modelos supervisados tradicionales poseen limitaciones que son analizadas en el artículo \cite{articulo1} tales como una dificultad para manejar datos de alta dimensionalidad, la dependencia del conocimiento de un experto, la baja capacidad para distinguir entre robo e irregularidades legítimas y, a veces, pueden ignorar la naturaleza temporal de los datos de consumo. Debido a estas limitaciones se motiva el uso de \emph{deep learning} o aprendizaje profundo. Este destaca en el manejo de problemas complejos de clasificación. Una red neuronal es una arquitectura de capa de neuronas que imitan el cerebro humano. Cada neurona se compone de entradas, ponderaciones, un sesgo y una salida. Si un valor de salida supera un umbral o sesgo preestablecido, la neurona se activa, pasando datos a la siguiente capa de la red.

Las principales técnicas supervisadas aplicadas a la detección de robo de electricidad, que se estudian en algunos de los trabajos de la literatura con más relevancia son: modelos clásicos como SVM \cite{articuloBase}, \cite{articulo5}, DT \cite{articuloBase}, regresión lineal y logística \cite{articuloBase}, KNN \cite{articulo1}, \cite{articuloBase}, \cite{articulo2} y \emph{Naïve Bayes} \cite{articulo1}, métodos ensemble como RF \cite{articuloBase}, \cite{articulo2}, \cite{articulo3} y \emph{XGBoost} \cite{articulo3} y, por último, destacan las redes neuronales, entre ellas el perceptrón multicapa (MLP) \cite{articulo3}, redes neuronales convolucionales (CNN) \cite{articuloBase}, \cite{articulo3} y redes neuronales recurrentes (RNN y LSTM) \cite{articulo1}, \cite{articuloBase}, \cite{articulo3}.
...
En esta sección se describen todos los aspectos relacionados a la gestión del proyecto: la metodología utilizada, la planificación, los costes, los riegos y el aseguramiento de la calidad.
...
La planificación consta de varias etapas secuenciales y con iteraciones, debido a la metodología y el carácter mixto del proyecto, que integra la experimentación en aprendizaje automático con la creación de un entorno web. La duración temporal estimada de todas estas etapas secuenciales están recogidas en el diagrama de Gantt de la figura~\ref{fig:gantt}. 

El análisis del problema incluye el estudio de la detección de fraude eléctrico, la revisión de técnicas actuales en el estado del arte y el examen del conjunto de datos elegido. La duración verdadera fue de tres semanas, mientras que la estimada era de cuatro. Se examinan distintos algoritmos de clasificación supervisada, incluyendo tanto los \emph{offline} como los \emph{online}, después de que el análisis, la investigación y la elección de técnicas de aprendizaje automático han concluido. Se eligen métodos de cada categoría según el problema para el conjunto de datos escogido. La duración estimada es de una semana, y la duración real también es de una semana.

La implementación de los modelos se lleva a cabo incluyendo la aplicación de modelos \emph{offline}. Para ello, se emplean seis métodos de aprendizaje \emph{offline} con el uso de bibliotecas especializadas y mediante la modificación de parámetros. La duración calculada es de dos semanas, pero la verdadera fue de tres a causa de dificultades en la optimización de alguno de los modelos. Después, para poner en práctica modelos \emph{online}, se emplean cinco métodos de aprendizaje \emph{online} mediante el uso de bibliotecas especializadas y la realización de ajustes a los parámetros. La duración real fue de dos semanas, al igual que la duración estimada. 

Luego de implementar los modelos, se procede a la evaluación comparativa de los mismos, para lo cual se elabora un protocolo estandarizado que emplea métricas utilizadas en la literatura. Se ponen en práctica los modelos bajo las mismas condiciones y se examinan los resultados. La duración real fue de dos semanas, al igual que la duración estimada. Además, del diseño e implementación del entorno web, se desarrolla la arquitectura de la web, incluyendo \emph{backend} mediante una \emph{API REST}, integración de modelos y visualización de resultados. La duración estimada y real son de cinco semanas. Por último,  las pruebas y validación del sistema, se realizan pruebas funcionales y no funcionales para garantizar el correcto funcionamiento del sistema y la integración entre componentes. La duración estimada es de una semana, al igual que la real.
...
El proyecto en su fase experimental se basa en un único conjunto de datos público, lo cuál puede limitar la generalización de los resultados. 
& Alta & Medio 
...
El aseguramiento de la calidad de este proyecto debe contemplar tanto la validez científica de los resultados como la robustez técnica del sistema implementado. Primero, se tienen en cuenta los distintos tipos de calidad. El primero, la calidad en la fase experimental garantiza que los resultados obtenidos en la comparativa entre modelos sean válidos y que se apliquen varias medidas como la definición de métricas adecuadas, la aplicación de un protocolo común de evaluación para todos los modelos, una clara separación entre datos de entrenamiento y las pruebas en los modelos \emph{offline} y el uso de evaluación \emph{prequential} en modelos \emph{online}. El segundo, la calidad del código y desarrollo software que garantiza que durante el desarrollo del sistema web se apliquen buenas prácticas de programación como el control de versiones, la modularización del código, la gestión estructurada de dependencias, la documentación clara y las pruebas. Y por último, la calidad en la integración entre modelos y sistema, que se aplica antes de realizar la integración de los modelos seleccionados en el entorno web. Y funciona de la siguiente manera, cada modelo se valida de manera independiente, se comprueban tiempos de inferencia, se verifica la correcta serialización y carga del modelo y, se realizan pruebas de integración.

Además, debe de tener reproducibilidad, para que los experimentos realizados puedan ser replicados bajo las mismas condiciones, obteniendo resultados consistentes y verificables. También,  hay que dejar claro varios aspectos técnicos del proyecto como el uso de versiones concretas de librerías, la definición del entorno virtual, la documentación del proceso de ejecución y una estructura clara del proyecto. Existen limitaciones de calidad debido a que el la fase experimental del proyecto solo se basa en único conjunto de datos, los resultados obtenidos pueden estar condicionados por sus propias características específicas. Esto se reconoce y se discute sobre ello en las conclusiones.
...
La detección de fraude eléctrico se plantea como un problema de clasificación supervisada, ya que él único conjunto de datos disponible y público incluye etiquetas que distinguen entre comportamientos normales y fraudulentos. Esto permite entrenar modelos capaces de aprender patrones de consumo asociados a distintos tipos de fraude y posteriormente clasificar nuevos registros de consumo eléctrico. Además, los métodos supervisados han demostrado obtener un elevado rendimiento en tareas de detección de pérdidas no técnicas dentro de redes eléctricas inteligentes \cite{articulo1}, \cite{articulo2}, \cite{articulo3}.
...
\begin{figure}[H]
    ...
    \caption{Curva sigmoidea de la regresión logística \cite{logistic-regression}.}
\end{figure}

Teniendo los siguientes datos: Eje $x$ es la variable de entrada o característica, el eje $y$ es la probabilidad estimada por el modelo y la curva muestra cómo cambia la probabilidad según el valor de entrada. La función solo devuelve valores entre 0 y 1 para la variable dependiente, al margen de los valores de la variable independiente. Así estima la regresión logística el valor de la variable dependiente. 
...
\begin{figure}[H]
    ...
    \caption{Árboles de decisión del algoritmo de Bosque Aleatorio \cite{Random Forest}.}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Proceso del \emph{Boosting} de gradiente \cite{Gradient Boosting}.}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Diagrama KNN \cite{k-Nearest Neighbors}.}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Funcionamiento de un Perceptrón Multicapa \cite{Perceptrón Multicapa}.}
\end{figure}
...
\noindent donde \(L\) es la función de pérdida y \(\theta\) es el conjunto de parámetros del modelo, pesos y sesgos. Una vez obtenido el gradiente, los parámetros del modelo se actualizan con el objetivo de minimizar el error. Este ajuste se realiza normalmente mediante el algoritmo de descenso por gradiente, según la expresión:

\[
\theta_{t+1} = \theta_t - \eta \nabla J(\theta_t)
,\]

\noindent donde \(\theta_t\) son los parámetros del modelo en la iteración \(t\), \(\theta_{t+1}\): son los parámetros actualizados en la siguiente iteración, \(\eta\) es la tasa de aprendizaje, que controla el tamaño de los ajustes y \(\nabla J(\theta_t)\) el gradiente de la función de pérdida en la iteración \(t\).
...
El perceptrón es uno de los modelos más simples del aprendizaje supervisado y puede emplearse tanto en entornos \emph{offline} como \emph{online}. En este trabajo se utiliza dentro del aprendizaje automático \emph{online} debido a su capacidad de aprendizaje incremental, permitiendo actualizar sus pesos progresivamente a medida que llegan nuevas instancias de datos, sin necesidad de reentrenar el modelo completo. Se selecciona como modelo base o \emph{baseline} por su simplicidad, bajo coste computacional y rápida capacidad de adaptación ante cambios en los patrones de consumo eléctrico. Además, servirá como referencia para comparar el comportamiento de técnicas más complejas utilizadas en este estudio.
...
\noindent donde \( b \) es el término de sesgo o \emph{bias}. A continuación, se aplica una función de activación que transforma el valor obtenido en una salida binaria:
...
\noindent donde \( \eta \) es la tasa de aprendizaje, que controla la magnitud del ajuste.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{imagenes/Perceptron.png}
    \caption{Funcionamiento del Perceptrón \cite{Perceptrón}.}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Funcionamiento del \emph{Adaptive random forest}.}
\end{figure}
...
\noindent siendo \emph{Precision}, la fórmula de la métrica de \emph{Precision}, y \emph{Recall}, la fórmula de la métrica de \emph{Recall} antes vistas. Se utiliza la media armónica en vez de la media aritmética porque esta última penaliza las grandes diferencias. En caso de alta precisión pero baja recuperación (o viceversa), el f1-score disminuirá significativamente, lo que refleja un desequilibrio.


\section{\emph{False positive rate}}
...
\noindent donde $TP$ (Verdaderos Positivos) representa los fraudes correctamente detectados, $FN$ (Falsos Negativos) los fraudes no detectados, $FP$ (Falsos Positivos) los usuarios normales clasificados como fraude y $TN$ (Verdaderos Negativos) los usuarios normales correctamente clasificados.
...
\noindent donde $\mathbf{p_o}$ es precisión observada del modelo (fórmula de la \emph{Accuracy}) y $\mathbf{p_e}$ es la proporción de acuerdo esperado por azar. $\mathbf{p_e}$ se calcula utilizando las probabilidades marginales de cada clase. En clasificación binaria se tiene:
...
\subsection{Modelos \emph{offline}}

Todos los modelos \emph{offline} implementados en este proyecto siguen un proceso común de preparación de datos, entrenamiento y evaluación. Este procedimiento permite garantizar que los distintos algoritmos se comparen bajo las mismas condiciones experimentales.
...
Tras el entrenamiento, se calcula la importancia de las variables utilizadas por el modelo como en el modelo anterior. En el caso de este modelo, la importancia de cada variable se calcula a partir de la reducción del error que produce cada característica al dividir los nodos de los árboles de decisión. Cuanto mayor sea esta reducción, mayor será la relevancia de la variable en el modelo. Las variables más importantes se almacenan en un archivo independiente y también se representan mediante un gráfico que muestra las características con mayor influencia en la detección de fraude.
...
\subsection{Modelos \emph{online}}
...
Como parámetro principal tiene \emph{random\_state=42}. Para llevar a cabo el entrenamiento del modelo y que permita la actualización incremental a medida que se procesan nuevos bloques de datos, es necesario especificar previamente el conjunto de clases del problema mediante los valores únicos de la variable objetivo. Dado que el Perceptrón no proporciona probabilidades de clasificación de forma directa, se obtiene una puntuación continua que representa la distancia de cada muestra al hiperplano de decisión, esta puntuación permite evaluar la capacidad discriminativa del modelo.


\subsubsection{Naive bayes incremental}

El modelo estima la probabilidad de pertenencia a cada clase a partir de la distribución de las características, asignando la clase con mayor probabilidad. En este trabajo se utiliza la variante \emph{Gaussian Naive Bayes} adecuada para variables numéricas continuas. Este modelo asume que cada característica sigue una distribución normal dentro de cada clase. A diferencia de los modelos lineales, este modelo no requiere escalado previo de las variables, ya que su funcionamiento se basa en la estimación de distribuciones estadísticas y no en distancias o márgenes de separación. Durante la evaluación, se utilizan directamente las probabilidades de pertenencia a la clase positiva obtenidas. 


\subsubsection{K vecinos más cercanos \emph{online}}

En esta implementación, el modelo se adapta a un escenario online mediante una estrategia de memoria incremental. El flujo de datos se divide en bloques o \emph{batches}, y cada nuevo bloque se clasifica utilizando únicamente las instancias vistas hasta ese preciso momento. Una vez realizadas las predicciones, el bloque actual se incorpora al conjunto de referencia, permitiendo que el modelo disponga de información más reciente en iteraciones posteriores. Además, dado que el algoritmo depende directamente del cálculo de distancias entre observaciones, se aplica un proceso de escalado previo, de forma que todas las variables contribuyan de manera equilibrada a la medida de similitud.
...
En general, los resultados obtenidos por los modelos de aprendizaje \emph{online} muestran un rendimiento algo más variable que los métodos \emph{offline}. Los modelos más simples presentan mayores dificultades para mantener un equilibrio adecuado entre capacidad de detección y control de falsos positivos, mientras que los métodos con mejor capacidad de adaptación y técnicas ensemble ofrecen un comportamiento bastante más sólido.

En primer lugar, el \emph{Perceptrón online} presenta un rendimiento moderado, alcanzando una exactitud del 68.5\% y una puntuación F1 de 0.622. Su precisión de 0.609 y \emph{recall} de 0.635 muestran una capacidad limitada tanto para detectar correctamente los casos de fraude como para evitar falsas alarmas. Además, su tasa de falsos positivos de 0.281 resulta un poco elevada, teniendo dificultades para distinguir adecuadamente entre las clases. El valor ROC-AUC de 0.716 confirma igualmente una capacidad de clasificación moderada. Estos resultados evidencian que, aunque el modelo puede adaptarse incrementalmente a nuevos datos, su simplicidad limita su rendimiento.

Por otro lado, el modelo \emph{Naive bayes incremental} obtiene el peor rendimiento global en términos de exactitud, con un valor de 49.3\%, así como un coeficiente Kappa muy reducido de 0.090, indicando un bajo nivel respecto a las etiquetas reales. Sin embargo, destaca por alcanzar el mayor \emph{recall} de todos los modelos \emph{online}, con un valor de 0.872, lo que indica una elevada capacidad para detectar casos fraudulentos. Esta capacidad se consigue a costa de una precisión considerablemente baja de 0.439 y una tasa de falsos positivos extremadamente alta de 0.768. En consecuencia, el modelo genera un gran número de falsas detecciones, lo que limita significativamente su utilidad práctica. Su valor ROC-AUC de 0.596 muestra además una capacidad discriminativa reducida.

El modelo \emph{kNN online} presenta un comportamiento bastante más equilibrado. Obtiene una exactitud del 89.3\% y una puntuación F1 de 0.863, manteniendo un buen equilibrio entre precisión de 0.902 y \emph{recall} de 0.828. Además, presenta una tasa de falsos positivos relativamente baja de 0.062 y una ROC-AUC de 0.897, lo que muestra una buena capacidad para diferenciar entre comportamientos normales y fraudulentos. Estos resultados muestran que la adaptación incremental del algoritmo kNN permite mantener un rendimiento competitivos.

En cuanto al modelo \emph{Hoeffding tree}, se observa un rendimiento similar al de \emph{kNN online}, aunque ligeramente inferior en la mayoría de métricas de evaluación. El modelo alcanza una exactitud del 87.4\% y una puntuación F1 de 0.834. Su precisión de 0.898 y \emph{recall} de 0.779 reflejan un comportamiento equilibrado, acompañado de una tasa de falsos positivos de 0.061. También, el valor ROC-AUC de 0.894 confirma una buena capacidad de clasificación.

Finalmente, el modelo \emph{Adaptive random forest} presenta el mejor rendimiento global entre todos los métodos \emph{online} evaluados. Destaca especialmente por alcanzar la mayor exactitud, con un 91.7\%, así como la mejor puntuación F1 de 0.891 y el coeficiente Kappa más elevado de 0.824. Además, combina una precisión muy alta de 0.960 con un \emph{recall} de 0.831, manteniendo la menor tasa de falsos positivos de todos los modelos \emph{online}, con un valor de 0.024. Su ROC-AUC de 0.959 confirma una excelente capacidad discriminativa.

Para analizar la capacidad de distinguir entre comportamiento normales o fraudes de los modelos, se representan las curvas ROC de forma conjunta para los métodos \emph{offline} y \emph{online}.
...
En la Figura \ref{fig:roc_online} se presentan las curvas ROC de los modelos \emph{online}. A diferencia del caso \emph{offline}, en este escenario se observa una mayor separación entre las curvas, lo que permite distinguir con mayor claridad el rendimiento de cada algoritmo. El modelo \emph{Adaptive random forest} destaca claramente como el mejor método \emph{online}, alcanzando el mayor valor de ROC-AUC y mostrando una curva más cercana al comportamiento óptimo. Le siguen el kNN \emph{online} y el \emph{Hoeffding tree}, que presentan un rendimiento competitivo y relativamente similar. En contraste, el Perceptrón \emph{online} y el \emph{Naive bayes} incremental muestran un comportamiento notablemente inferior, especialmente este último, cuya curva se aproxima en mayor medida a la línea base, indicando una menor capacidad de discriminación.
...
En primer lugar, el modelo \emph{Random forest} identifica como variables más relevantes aquellas relacionadas con el consumo eléctrico en distintos componentes del sistema. En particular, destacan  \emph{InteriorEquipment:Electricity},  \emph{InteriorLights:Electricity} y  \emph{Fans:Electricity}, lo que indica que el consumo energético interno del edificio es un factor clave en la detección de comportamientos anómalos. Además, variables relacionadas con el consumo de gas y agua, como  \emph{Water Heater:WaterSystems:Gas} e  \emph{InteriorEquipment:Gas}, también presentan una contribución significativa. Esto sugiere que el fraude no solo se manifiesta en el consumo eléctrico directo, sino también en patrones de consumo energético global.
...
En este capítulo se va a explicar y detallar al completo los aspectos relacionados al desarrollo del prototipo como los requisitos, modelo conceptual, casos de uso, arquitectura, base de datos, entorno de desarrollo, tecnologías, control de versiones y despligue.

Para definir el desarrollo del prototipo \emph{software}, se ha realizado un análisis del problema de detección de fraude en el consumo eléctrico y de las necesidades que tendría una empresa energética o analista encargado de supervisar los datos de consumo. El objetivo del sistema es proporcionar una herramienta que permita analizar datos de consumo energético y aplicar modelos de aprendizaje automático para detectar posibles casos de fraude. Además, se ofrece un entorno web que facilite la visualización de los resultados y la comparación entre distintos modelos de clasificación, permitiendo interpretar de forma clara el rendimiento de cada uno de ellos.
...
En esta sección se van a describir cada uno de los requisitos del prototipo web. Estos requisitos se dividen en funcionales, no funcionales y de información.
...
RF3 & Integración de modelos de detección de fraude. & El sistema debe integrar distintos modelos de clasificación previamente entrenados para detectar posibles casos de fraude en el consumo eléctrico. \\ \hline

RF4 & Evaluación de modelos de aprendizaje automático. & El sistema debe permitir evaluar el rendimiento de los modelos utilizados mediante métricas de clasificación durante la fase experimental del proyecto. \\ \hline

RF5 & Validación experimental de modelos. & El sistema debe permitir realizar procesos de validación cruzada sobre los modelos desarrollados para analizar su estabilidad y capacidad de generalización durante la fase experimental. \\ \hline

RF6 & Visualización de resultados y estadísticas. & El sistema debe mostrar gráficamente curvas de consumo, resultados de predicción y estadísticas agregadas mediante el entorno web. \\ \hline

RF7 & Uso de múltiples modelos de detección. & El sistema debe utilizar distintos modelos de aprendizaje automático adaptados a escenarios de predicción histórica y procesamiento en tiempo real. \\ \hline

RF8 & Consulta de resultados almacenados. & El sistema debe permitir consultar archivos importados, curvas de consumo y resultados de predicción previamente almacenados en la base de datos. \\ \hline

RF9 & Autenticación y control de acceso. & El sistema debe permitir a los usuarios registrarse e iniciar sesión mediante autenticación basada en \emph{tokens} JWT, restringiendo el acceso únicamente a los datos asociados a cada usuario. \\ \hline
...
RNF1 & Rendimiento. & El sistema debe procesar archivos de consumo eléctrico y generar predicciones en tiempos adecuados para un entorno interactivo web. \\ \hline

RNF2 & Escalabilidad. & La arquitectura del sistema debe permitir incorporar nuevos modelos de aprendizaje automático y ampliar funcionalidades sin modificar significativamente la estructura existente. \\ \hline

RNF3 & Usabilidad. & La interfaz web debe ser sencilla e intuitiva, permitiendo a los usuarios importar archivos, visualizar curvas y consultar predicciones sin necesidad de conocimientos técnicos avanzados. \\ \hline
...
RNF6 & Fiabilidad. & El sistema debe generar resultados consistentes y garantizar un acceso seguro y controlado a los datos asociados a cada usuario. \\ \hline

RNF7 & Seguridad. & El sistema debe restringir el acceso a la información mediante autenticación basada en tokens JWT y mecanismos de control de acceso por usuario. \\ \hline
...
RI1 & Datos de consumo energético. & El sistema debe gestionar y almacenar datos de consumo eléctrico importados mediante archivos CSV para su posterior análisis y predicción de fraude. \\ \hline

RI2 & Etiquetas de clasificación. & El sistema debe manejar etiquetas de clasificación asociadas a consumos normales y posibles casos de fraude eléctrico utilizadas durante la fase de entrenamiento y evaluación de los modelos. \\ \hline

RI3 & Resultados de predicción. & El sistema debe almacenar los resultados generados por los modelos de detección de fraude, incluyendo tipo de modelo utilizado, resultado de clasificación y probabilidad estimada de fraude. \\ \hline

RI4 & Datos de experimentación. & El sistema debe permitir documentar y comparar los resultados obtenidos durante la fase experimental de evaluación de modelos de aprendizaje automático. \\ \hline

RI5 & Datos de usuarios. & El sistema debe almacenar información básica de los usuarios registrados para gestionar autenticación, autorización y acceso a los recursos asociados a cada cuenta. \\ \hline

RI6 & Protección de datos. & Las credenciales y datos de acceso de los usuarios deben almacenarse de forma segura mediante técnicas de cifrado y mecanismos de autenticación, limitando su uso exclusivamente a la gestión del acceso al sistema. \\ \hline

RI7 & Asociación de datos por usuario. & El sistema debe relacionar archivos, curvas de consumo y predicciones con el usuario propietario para garantizar el aislamiento de la información entre cuentas. \\ \hline
...
El modelo conceptual de datos gestiona los usuarios, archivos de consumo eléctrico  que contienen curvas y resultados de predicción de fraude. La clase \emph{Usuario} son los operadores autenticados que acceden al entorno web, importan archivos y realizan predicciones de fraude. Cada usuario puede importar varias instancias de la clase \emph{ArchivoConsumo}, que contienen datos sobre los conjuntos de información incorporados en el sistema. Cada archivo importado está formado por una o más entidades \emph{CurvaConsumo}, las cuales representan las curvas de consumo eléctrico individuales generadas con los datos del archivo. Para realizar análisis históricos y simulaciones en tiempo real, es posible analizar estas curvas con los modelos predictivos que están incluidos en el sistema. La clase \emph{Predicción} guarda los resultados de cada análisis. Esta entidad contiene información sobre el modelo que se usó, la clase de predicción que se efectuó, el resultado logrado, la probabilidad calculada de fraude y el día en que se llevó a cabo la clasificación.

\medskip

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{imagenes/mcd_TFG.png}
...
El diagrama muestra que el operador tiene que iniciar sesión para entrar al sistema. Su función principal es la de realziar una predicción de fraude y consultar estadísticas, lo que conlleva importar un archivo CSV que contenga curvas de consumo de electricidad la primera vez, después no hace falta importarlo ya que se almacena en el sistema.

Para el administrador, se han identificado dos principales casos de uso que tienen como objetivo la conservación del sistema. El primero implica la administración de modelos, lo que posibilita añadir nuevos modelos entrenados o actualizar los existentes. El segundo caso de uso tiene que ver con la administración de usuarios, a través de la cual el administrador tiene la capacidad de crear, modificar o borrar usuarios y manejar sus permisos para acceder. Estos casos de uso, que son tareas de mantenimiento y configuración esenciales para asegurar un funcionamiento adecuado, no se incluyen en el flujo principal del sistema.


\subsection{Descripción de escenarios del operador}

A continuación, se desarrolla la descripción de los escenarios de casos de uso del operador. Para todos los casos de uso asociados al operador se considera como precondición que el usuario ha iniciado sesión correctamente en el sistema. Por tanto, el caso de uso \emph{Iniciar sesión} no se repite en la descripción de cada escenario, aunque se entiende como requisito previo para acceder a las funcionalidades del sistema. Además, para los casos de usos de \emph{Realizar predicción de fraude histórico}, \emph{Realizar predicción de fraude en tiempo real} y \emph{Consultar estadísticas} se considera como precondición que exista, al menos, un archivo de consumo previamente importado, es decir el caso de uso de \emph{Importar archivo de consumo eléctrico}.
...
\begin{itemize}[leftmargin=1.5cm]
    ...
    \item Escenario alternativo (error): Curva de consumo no válida.
    \item Escenario alternativo (error): Modelo predictivo no disponible.
\end{itemize}
...
\begin{enumerate}[leftmargin=1.5cm]
    ...
    \item El operador selecciona un archivo de consumo previamente importado.
    \item El sistema muestra las curvas asociadas al archivo seleccionado.
    \item El operador selecciona una curva de consumo.
    \item El sistema obtiene los datos asociados a la curva seleccionada.
    \item El sistema aplica el modelo predictivo \emph{offline}.
    \item El sistema genera la clasificación de la curva.
    \item El sistema muestra el resultado de la predicción indicando si el consumo corresponde a un posible fraude o a un caso normal y la probabilidad asociada a la predicción realizada.
\end{enumerate}
...
5a. Curva de consumo no válida.
\begin{enumerate}[leftmargin=1.5cm]
    \item El sistema detecta que la curva contiene datos inválidos o incompatibles.
    \item El sistema muestra un mensaje de error indicando que la predicción no puede realizarse.
\end{enumerate}

\medskip

6a. Modelo predictivo no disponible.
\begin{enumerate}[leftmargin=1.5cm]
    \item El sistema detecta que el modelo predictivo histórico no está disponible.
    \item El sistema muestra un mensaje de error indicando que no es posible realizar la predicción.
\end{enumerate}
...
\begin{itemize}[leftmargin=1.5cm]
    ...
    \item Escenario alternativo (error): Curva de consumo no válida.
    \item Escenario alternativo (error): Modelo predictivo no disponible.
    \item Escenario alternativo (error): Error durante la generación de predicciones \emph{online}.
\end{itemize}
...
\begin{enumerate}[leftmargin=1.5cm]
    ...
    \item El sistema muestra las curvas asociadas al archivo seleccionado.
    \item El operador selecciona una curva de consumo.
    \item El sistema inicia la simulación del flujo de datos en tiempo real.
    \item El sistema obtiene los datos asociados a la curva seleccionada.
    \item El sistema procesa secuencialmente cada punto de consumo de la curva.
    \item El sistema aplica el modelo predictivo online sobre cada nuevo punto recibido.
    \item El sistema actualiza dinámicamente la predicción de fraude conforme avanza el flujo de datos.
    \item El sistema muestra en tiempo real el estado de la predicción, indicando si el comportamiento corresponde a un posible fraude o a un consumo normal, junto con la probabilidad asociada.
\end{enumerate}
\medskip

\noindent Extensiones:

6a. Curva de consumo no válida.
\begin{enumerate}[leftmargin=1.5cm]
    \item El sistema detecta que la curva contiene datos inválidos, incompletos o incompatibles.
    \item El sistema interrumpe la simulación del flujo \emph{online} y muestra un mensaje de error indicando que no es posible realizar las predicciones.
\end{enumerate}

\medskip

8a. Modelo predictivo no disponible.
\begin{enumerate}[leftmargin=1.5cm]
    \item El sistema detecta que el modelo predictivo \emph{online} no está disponible.
    \item El sistema cancela el proceso de predicción en tiempo real y muestra un mensaje de error indicando que no es posible realizar las predicciones.
\end{enumerate}

\medskip

9a. Error durante la generación de predicciones \emph{online}.
\begin{enumerate}[leftmargin=1.5cm]
    \item El sistema detecta un error durante el procesamiento secuencial de los datos.
    \item El sistema detiene la simulación del flujo \emph{online} y muestra un mensaje de error indicando que no ha sido posible completar la simulación.
\end{enumerate}
...
\begin{itemize}[leftmargin=1.5cm]
    ...
    \item Escenario alternativo (excepción): El operador cancela la importación.
\end{itemize}
...
\begin{enumerate}[leftmargin=1.5cm]
    ...
    \item El operador confirma la selección.
    \item El sistema comprueba que el archivo tiene un formato válido.
    \item El sistema lee el contenido del archivo.
    \item El operador selecciona el botón de importar.
    \item El sistema almacena el archivo importado y las curvas de consumo asociadas.
    \item El sistema confirma que la importación se ha realizado correctamente.
\end{enumerate}
...
5a. Formato de archivo no válido.
\begin{enumerate}[leftmargin=1.5cm]
    \item El sistema detecta que el archivo seleccionado no corresponde a un archivo válido.
    \item El sistema muestra un mensaje de error indicando que únicamente se permiten archivos CSV.
\end{enumerate}

\medskip

6a. El archivo no puede procesarse correctamente.
\begin{enumerate}[leftmargin=1.5cm]
    \item El sistema detecta que el archivo está vacío, no puede leerse o contiene datos incompatibles.
    \item El sistema muestra un mensaje de error indicando que no ha sido posible procesar el archivo.
\end{enumerate}

\medskip

1a-3a. Cancelar importación de archivo de curvas de consumo.
\begin{enumerate}[leftmargin=1.5cm]
    \item El operador cancela la selección del archivo.
    \item El sistema cancela la importación sin almacenar información.
\end{enumerate}

\medskip

\subsubsection{Consultar estadísticas}

\noindent Cuarto caso de uso: Consultar estadísticas.

\begin{itemize}[leftmargin=1.5cm]
    \item Escenario principal (éxito): Consultar estadísticas.
    \item Escenario alternativo (error): El archivo no contiene curvas de consumo.
    \item Escenario alternativo (error): Error al generar las estadísticas.
\end{itemize}

\medskip

\noindent Escenario principal (éxito): Consultar estadísticas.

\begin{enumerate}[leftmargin=1.5cm]
    \item El operador accede al inicio.
    \item El operador selecciona un archivo de consumo previamente importado.
    \item El sistema obtiene las curvas asociadas al archivo seleccionado.
    \item El sistema genera las estadísticas y carga las gráficas correspondientes mientras muestra por pantalla un símbolo de carga.
    \item El sistema muestra las gráficas y los resultados estadísticos asociados al archivo.
\end{enumerate}
...
3a. El archivo no contiene curvas de consumo.
\begin{enumerate}[leftmargin=1.5cm]
    \item El sistema detecta que el archivo seleccionado no contiene curvas registradas.
    \item El sistema muestra un mensaje indicando que no existen datos disponibles para generar estadísticas.
\end{enumerate}

\medskip

4a. Error al generar las estadísticas.
\begin{enumerate}[leftmargin=1.5cm]
    \item El sistema detecta un error durante la generación de estadísticas o la carga de las gráficas.
    \item El sistema muestra un mensaje indicando que no ha sido posible generar las estadísticas del archivo seleccionado.
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.5cm]
    ...
    \item El sistema valida las credenciales introducidas y permite el acceso al sistema.
    \item El operador accede al entorno web del sistema.
\end{enumerate}
...
\noindent Sexto caso de uso: Cerrar sesión.
...
\begin{enumerate}[leftmargin=1.5cm]
    \item El operador selecciona la opción cerrar sesión.
    \item El sistema finaliza la sesión activa del operador.
    \item El sistema elimina las credenciales de acceso almacenadas.
    \item El sistema redirige al operador a la pantalla de inicio de sesión.
\end{enumerate}
...
2a. Error al cerrar sesión.
\begin{enumerate}[leftmargin=1.5cm]
    \item El sistema detecta que no ha sido posible finalizar correctamente la sesión.
    \item El sistema muestra un mensaje de error indicando que no fue posible cerrar la sesión del operador.
\end{enumerate}
...
Ahora es el turno de los dos casos de uso del administrador, estos se consideran tareas de mantenimiento del sistema.

\medskip

\subsubsection{Gestionar modelos}

\noindent Séptimo caso de uso: Gestionar modelos.

\begin{itemize}[leftmargin=1.5cm]
    \item Escenario principal (éxito): El administrador actualiza correctamente los modelos entrenados.
    \item Escenario alternativo (error): El modelo no tiene el formato esperado.
\end{itemize}

\medskip

\noindent Escenario principal (éxito): Gestionar modelos.
\begin{enumerate}[leftmargin=1.5cm]
    \item El administrador accede a los recursos internos del sistema.
    \item El administrador sustituye o añade modelos entrenados.
...
2a. Modelo no válido.
\begin{enumerate}[leftmargin=1.5cm]
    \item El administrador incorpora un modelo con formato incorrecto o incompatible.
    \item El sistema no puede utilizar correctamente dicho modelo.
    \item El administrador corrige el modelo y repite la operación.
\end{enumerate}
...
\noindent Octavo caso de uso: Gestionar usuarios.
...
La arquitectura del sistema sigue un modelo cliente-servidor basado en una aplicación web y una \emph{API REST}. El sistema permite importar archivos que tienen curvas de consumo eléctrico, ejecutar predicciones de fraude y visualizar resultados de manera interactiva a través de una interfaz web. El cliente web representa el punto de acceso para el operador, mientras que la interfaz es desarrollada utilizando \emph{HTML, CSS} y \emph{JavaScript}, permitiendo realizar todas las operaciones permitidas. La comunicación entre la interfaz y el servidor se realiza mediante peticiones \emph{HTTP} a una \emph{API REST} desarrollada con \emph{FastAPI}. La \emph{API} se encarga de recibir las solicitudes del cliente, validar la autenticación de usuarios, procesar los datos recibidos y devolver las respuestas correspondientes..

La lógica de negocio se encuentra organizada en servicios de predicción, responsables de cargar los modelos entrenados, realizar el preprocesamiento de datos y ejecutar las inferencias. El sistema tiene dos flujos principales de predicción. El primero corresponde a la predicción histórica, basada en el algoritmo \emph{XGBoost}, utilizada para analizar curvas de consumo almacenadas previamente. El segundo corresponde a la predicción en tiempo real, basada en \emph{Adaptive Random Forest}, orientada al procesamiento continuo de datos. El flujo en tiempo real se simula a partir de datos previamente almacenados, enviados de forma secuencial a través de una cola interna.

La persistencia de la información se realiza mediante una base de datos \emph{MySQL}, encargada de almacenar usuarios, credenciales cifradas, archivos importados, curvas de consumo registradas y predicciones realizadas por los usuarios. Los modelos entrenados y las estructuras auxiliares necesarias para la predicción se cargan desde el almacenamiento interno del servidor durante la inicialización de los servicios de predicción. La Figura~\ref{fig:arquitectura_sistema} muestra el esquema general de la arquitectura implementada.
...
Se crea una base de datos relacional donde se guarda toda la información de los usuarios para controlar su acceso en el sistema, de los archivos importados por ellos, las curvas de consumo de los archivos y las predicciones realizadas. Estos datos están recogidos en 4 tablas, las cuales se llaman respectivamente \texttt{usuarios}, \texttt{archivos\_consumo}, \texttt{curvas\_consumo} y \texttt{predicciones}. En la Figura~\ref{fig:diseno_bd} se muestra el diseño de la base de datos utilizada por el sistema.
...
La tabla \texttt{usuarios} está formada por varios elementos como un identificador único, un nombre de usuario, una contraseña almacenada mediante hash, un rol dentro del sistema, un estado de activación y una fecha de creación. El uso de contraseñas cifradas mediante hash evita tener contraseñas en texto plano, así se consigue reducir los casos de accesos no autorizados. El campo de estado permite desactivar usuarios sin necesidad de eliminarlos físicamente, conservando así la integridad de la información. En la Figura~\ref{fig:tabla_usuarios} se muestran sus atributos junto con detalles como el tipo de dato.
...
La tabla \texttt{archivos\_consumo} se compone de su identificador privado, del identificador del usuario para saber el usuario que importó el archivo, del nombre del archivo, de la fecha de importación, del total de curvas que dispone y de los datos que se utilizan para almacenar estadísticas y resultados generados automáticamente tras la importación de un archivo de curvas de consumo. En la Figura~\ref{fig:tabla_archivos} se muestran sus atributos junto con detalles como el tipo de dato.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/tabla_archivos.png}
\caption{Diseño de la tabla de archivos.}
\label{fig:tabla_archivos}
\end{figure}

\medskip

La tabla \texttt{curvas\_consumo} almacena el identificador privado de la curva, el identificador del archivo para saber a cual pertenecen, su identificador público para poder ser seleccionada y buscada por el operador, los datos de consumo de cada curva y la fecha de registro de la curva. En la Figura~\ref{fig:tabla_curvas} se muestran sus atributos junto con detalles como el tipo de dato.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/tabla_curvas.png}
\caption{Diseño de la tabla de curvas.}
\label{fig:tabla_curvas}
\end{figure}

\medskip

La tabla \texttt{predicciones} se forma por el identificador privado de la predicción, el identificador de la curva que se predice, los tipos de modelos y predicciones realizadas en cada predicción, los resultados junto a la probabilidad de fraude de la predicción y la fecha en la que se realiza. En la Figura~\ref{fig:tabla_predicciones} se muestran sus atributos junto con detalles como el tipo de dato.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/tabla_predicciones.png}
\caption{Diseño de la tabla de predicciones.}
\label{fig:tabla_predicciones}
\end{figure}


\section{Entorno de desarrollo}

El desarrollo se realiza con un equipo con \emph{Windows} junto con \emph{Python} como el lenguaje de programación principal para el desarrollo del \emph{back-end} y de los servicios de predicción. Esta alternativa es muy buena para el proyecto ya que permite integrar la lógica del sistema con los modelos de aprendizaje automático obtenidos en la etapa experimental, todo ello dentro del mismo entorno. Para crear la interfaz web, se utilizan lenguajes como \emph{JavaScript}, \emph{CSS} y \emph{HTML} y son los que permiten desarrollar las páginas de interacción con el operador, ver gráficos y gestionar dinámicamente las predicciones del sistema.

Se crea un entorno virtual a través de \emph{venv} para juntar las dependencias del proyecto. Las bibliotecas necesarias para la aplicación se mantienen separadas de otras instalaciones de \emph{Python} que se encuentran en el equipo o servidor, con lo cual se evitan conflictos entre versiones y se simplifica la replicación del entorno en otros equipos o en el despliegue. Se ha elegido \emph{Visual Studio Code} como el entorno de desarrollo integrado, que se emplea para la edición del código fuente, la estructuración del proyecto y la ejecución de comandos en la terminal integrada. 

Para gestionar e instalar las dependencias, se utiliza \emph{pip}. Este software crea el archivo de texto \emph{requirements}, que contiene una lista de todas las librerías necesarias para que el sistema funcione con normalidad. En la etapa de desarrollo, se utiliza el servidor \emph{Uvicorn}, que es compatible con \emph{FastAPI}, para ejecutar la aplicación. Para administrar la base de datos se emplean \emph{XAMPP}, \emph{MySQL} y \emph{phpMyAdmin} que ayudan con la creación y administración de la base de datos Por último, se emplea \emph{Git} para controlar las versiones, lo que posibilita el seguimiento del avance del código fuente y la supervisión de las modificaciones efectuadas durante la edificación del sistema.


\section{Tecnologías utilizadas}

Para construir el sistema se emplean diversas tecnologías en las que cada una se encarga de una tarea distinta. Se emplea \emph{FastAPI} como base fundamental para crear la \emph{API REST}. Esta aporta que los \emph{endpoints} funcionen y hagan su tarea correctamente, la validación de los parámetros de entrada y la creación automática de una documentación del sistema que es interactiva. Gracias a esto el operador puede comprobar las funcionalidades disponibles a través de una interfaz a la que se accede por medio del navegador. La aplicación desarrollada con \emph{FastAPI} se ejecuta usando el servidor \emph{Uvicorn}. Este servidor atiende las peticiones HTTP recibidas por la \emph{API} y ejecuta los \emph{endpoints} definidos en el sistema.

Para crear la interfaz web, se emplean tecnologías estándar del lado cliente, como \emph{JavaScript}, \emph{CSS} y \emph{HTML}. Estas ayudan en la creación de las diversas visualizaciones del sistema, que incluyen los paneles de estadísticas, las páginas de predicción en tiempo real, de autenticación y las de predicción histórica. La biblioteca \emph{Chart.js} se utiliza para crear representaciones visuales de los resultados y métricas del sistema, incluyendo diagramas circulares, histogramas y gráficos de barras. Estos se muestran en el \emph{dashboard} de estadísticas.
...
Se emplea \emph{MySQL} como sistema gestor de base de datos para administrar usuarios, archivos, curvas de consumo y predicciones. La \emph{API} y la base de datos se comunican mediante \emph{PyMySQL}, lo que permite realizar consultas a través del código \emph{Python}. Para una buena seguridad en las contraseñas se emplean funciones \emph{hash} mediante el algoritmo \emph{bcrypt} y la biblioteca \emph{Passlib}. Estos ayudan a la verificación de las contraseñas que el usuario ingresa, al compararlas con el hash guardado en la base de datos. Así, se evita almacenar las contraseñas como texto plano. Además, la autenticación de usuarios se basa en el uso de \emph{JSON Web Tokens (JWT)}. Tras iniciar sesión correctamente, el sistema genera un token que se utiliza para validar el acceso a los \emph{endpoints} protegidos de la \emph{API}, permitiendo mantener sesiones seguras sin almacenar información sensible en el servidor.
...
\section{Control de versiones y Código fuente}
\label{sec:repositorio}
El control de versiones del proyecto se realiza mediante \emph{Git}, permitiendo registrar todos los cambios realizados durante el desarrollo del sistema y facilitando la trazabilidad del código fuente El código fuente completo del proyecto se encuentra disponible en un repositorio privado/público alojado en \emph{GitHub}, accesible mediante el siguiente enlace:

\begin{center}
 \url{https://github.com/Jorgesi27/TFG_FraudeElectricoML}
\end{center}

El uso de \emph{GitHub} permite mantener un historial completo de cambios, facilitando la gestión evolutiva del sistema, la recuperación de versiones anteriores y el seguimiento de las distintas funcionalidades implementadas durante el desarrollo.


\section{Despliegue del sistema}

En esta sección se describe el procedimiento necesario para desplegar y ejecutar el prototipo desarrollado, y se muestra el despligue en el servidor de \emph{AWS}. El objetivo es permitir la replicación del entorno de ejecución en otro equipo, incluyendo la instalación de dependencias, configuración de la base de datos y puesta en marcha de la aplicación web. Además, de ver las configuraciones elegidas para su despliegue en la nube.


\subsection{Replicación del entorno local}

Para ejecutar el sistema es necesario disponer de un entorno con soporte para \emph{Python} y un sistema gestor de base de datos \emph{MySQL}. Además, se requiere un navegador web moderno para acceder a la interfaz gráfica del prototipo. Los requisitos mínimos del entorno son los siguientes: \emph{Python} 3.10.11, \emph{MySQL Server}, \emph{Git} (opcional), navegador web moderno y un sistema operativo \emph{Windows} o \emph{Linux}.

El proyecto puede obtenerse mediante clonación del repositorio utilizando Git o descargando directamente el código fuente en formato comprimido. Véase Sección \ref{sec:repositorio}.

La clonación del repositorio puede realizarse mediante el siguiente comando: \texttt{git clone https://github.com/Jorgesi27/TFG\_FraudeElectricoML.git}

Una vez descargado el proyecto, debe accederse al directorio raíz del sistema: \texttt{cd TFG\_FraudeElectricoML}

Para aislar las dependencias del proyecto se utiliza un entorno virtual de Python. La creación del entorno virtual se realiza mediante el siguiente comando: \texttt{python -m venv venv}

\noindent Activación del entorno virtual en Windows: \texttt{venv\textbackslash Scripts\textbackslash activate}

\noindent Activación del entorno virtual en Linux: \texttt{source venv/bin/activate}

Una vez activado el entorno virtual, deben instalarse las bibliotecas necesarias definidas en el archivo \emph{requirements.txt}. \texttt{pip install -r requirements.txt}

Para crear y configurar la base de datos utilizada por el sistema es necesario utilizar \emph{MySQL}. En primer lugar, debe crearse la base de datos principal empleada por la aplicación. \texttt{CREATE DATABASE tfg\_fraudeelectrico;}

\noindent Posteriormente, debe seleccionarse la base de datos creada. \texttt{USE tfg\_fraudeelectrico;}

A continuación, se muestran las sentencias necesarias para crear las tablas utilizadas por el sistema. La tabla \texttt{usuarios} almacena la información relacionada con autenticación y gestión de usuarios del sistema.

\begin{lstlisting}[caption={Tabla de usuarios.}, label={lst:tabla_usuarios}]
...
\medskip
\noindent La tabla \texttt{archivos\_consumo} almacena los archivos importados por cada usuario.

\begin{lstlisting}[caption={Tabla de archivos.}, label={lst:tabla_archivos}]
...
\medskip
\noindent La tabla \texttt{curvas\_consumo} almacena las curvas eléctricas individuales extraídas de los archivos importados.

\begin{lstlisting}[caption={Tabla de curvas.}, label={lst:tabla_curvas}]
...
\medskip
\noindent La tabla \texttt{predicciones} almacena el historial de predicciones realizadas por los modelos del sistema.

\begin{lstlisting}[caption={Tabla de predicciones.}, label={lst:tabla_predicciones}]
...
Tras la creación de la base de datos, deben configurarse las credenciales de acceso utilizadas por la aplicación. Para ello, el sistema utiliza un archivo de variables de entorno denominado \texttt{.env}, situado en la raíz del proyecto. Este archivo permite separar la configuración sensible del código fuente, evitando almacenar directamente las credenciales dentro de la aplicación. El archivo \texttt{.env} debe contener los parámetros necesarios para establecer la conexión con la base de datos \emph{MySQL}, incluyendo el host, el usuario, la contraseña y el nombre de la base de datos utilizada por el sistema. Un ejemplo de configuración es el siguiente:

\begin{lstlisting}[language=bash, caption={Configuración de variables de entorno}]
DB_HOST=localhost
DB_USER=root
DB_PASSWORD=
DB_NAME=tfg_fraudeelectrico
\end{lstlisting}

Estas variables son cargadas automáticamente por la aplicación mediante la biblioteca \emph{python-dotenv} desde el módulo encargado de la conexión con la base de datos (\texttt{database.py}). Además, el sistema requiere disponer de los modelos previamente entrenados utilizados durante la fase experimental. Dichos modelos se encuentran almacenados en formato \texttt{.pkl} dentro del directorio \texttt{models/}.

Por otra parte, los archivos utilizados para realizar predicciones históricas o simulaciones en tiempo real deben almacenarse dentro del directorio \texttt{data/}. El sistema accede a estos archivos durante la ejecución para procesar las curvas de consumo eléctrico importadas por el operador.

Debido a que las contraseñas no se almacenan en texto plano, es necesario generar previamente el \emph{hash} de la contraseña antes de insertar un usuario en la base de datos. Para ello, se utiliza la biblioteca \emph{Passlib} junto con el algoritmo \emph{bcrypt}. El siguiente \emph{script} permite generar un hash válido a partir de una contraseña introducida por consola:

\begin{lstlisting}[language=Python, caption={Generación de hash para contraseñas}]
...
Una vez generado el \emph{hash}, el usuario puede insertarse manualmente en la tabla \texttt{usuarios}, usando el usuario y contraseña con \emph{hash} del  \emph{script} anterior, mediante una sentencia \emph{SQL} como la siguiente:

\begin{lstlisting}[language=SQL, caption={Inserción manual de un usuario}]
INSERT INTO usuarios(nombre_usuario, password_hash, rol)
VALUES('usuario', 'hash sacado del script', 'administrador');
\end{lstlisting}
...
Tras la ejecución del servidor, la interfaz web y la documentación automática de la API estarán disponibles desde el navegador web. Por defecto, el servidor se ejecuta sobre el puerto \texttt{8000}. Por tanto, la aplicación web puede accederse desde la dirección: \texttt{http://localhost:8000}. Y la documentación interactiva generada automáticamente por \emph{FastAPI} se encuentra disponible en: \texttt{http://localhost:8000/docs}.


\subsection{Despliegue en servidor AWS}
...
% CAPÍTULO 7
% =========================
\chapter{Validación del prototipo web}



\section{Entorno de validación}
...
En primer lugar, se examinan las conclusiones de la etapa experimental en relación con los modelos y métricas que se han estudiado. Se pueden sacar varias conclusiones relevantes acerca de la capacidad de generalización, el rendimiento y la aplicabilidad de los modelos a partir de los resultados obtenidos.

Los modelos \emph{offline} son más eficientes que los modelos \emph{online}. Esto sucede porque estos algoritmos cuentan con la totalidad de los datos durante el entrenamiento, lo cual les posibilita calibrar sus parámetros de manera más exacta y captar conexiones complejas entre las variables. Los modelos que se basan en técnicas de ensamble, como \emph{MLP}, \emph{Random Forest}, \emph{Gradient Boosting} y \emph{XGBoost}, son notables. El algoritmo con mejor rendimiento es el \emph{XGBoost}, porque combina una precisión alta con una capacidad de detección de fraude también alta, manteniendo al mismo tiempo una tasa de falsos positivos muy pequeña. Esto puede explicarse por que está basado en \emph{boosting}, donde cada nuevo modelo corrige los errores de los anteriores. Los modelos más simples, como la Regresión Logística, presentan un rendimiento bastante inferior. Esto se debe a su capacidad limitada para modelar relaciones no lineales en los datos. Aunque este modelo se usa como una base simple para comparar el resto de modelos más avanzados, no resulta útil para la detección de fraude eléctrico.

En los modelos \emph{online} se observan comportamientos más variados. El modelo \emph{Adaptive Random Forest} destaca por encima del resto. Este resultado demuestra que es posible obtener un rendimiento competitivo en flujos de datos mediante el uso de técnicas de ensemble adaptativas. Los modelos como \emph{kNN online} y \emph{Hoeffding Tree} muestran un rendimiento aceptable, aunque inferior. Los modelos como \emph{Perceptrón online} y el \emph{Naive Bayes incremental} tienen limitaciones importantes, especialmente en la capacidad para distinguir entre comportamientos normales o fraudulentos.

Los resultados muestran que existe una relación entre la capacidad de predicción y la de adaptación al flujo de datos. Los modelos \emph{offline} logran mejores resultados en capacidad para distinguir entre comportamientos normales y fraudes, así como en precisión. Sin embargo, los modelos \emph{online} posibilitan el trabajo en entornos dinámicos en los que la información se recibe constantemente; esto es muy relevante para las aplicaciones en tiempo real. En este estudio se subraya la relevancia de elegir el modelo apropiado según el contexto del problema. Modelos como \emph{XGBoost} son la opción más adecuada en situaciones donde se cuenta con todas las informaciones disponibles y el objetivo es optimizar los resultados. No obstante, en contextos en los que se producen datos de manera constante, algoritmos como el \emph{Adaptive Random Forest} brindan una opción efectiva al fusionar un rendimiento competitivo con la adaptabilidad.

Además de la fase experimental, se desarrolla un prototipo funcional orientado a la detección de fraude eléctrico, integrando distintos componentes software para el procesamiento de datos, ejecución de predicciones y visualización de resultados dentro de una aplicación web completa. Este desarrollo del prototipo permite validar la integración práctica de estos modelos dentro de un sistema software completo permitiendo trasladar el trabajo experimental a un entorno de uso más cercano a una aplicación real.

Sin embargo, el sistema tiene algunas limitaciones que proceden del entorno experimental empleado. Entre ellas destacan la dependencia de los datos disponibles, la simulación del flujo en tiempo real y la ausencia de integración con infraestructuras eléctricas reales. Además, el rendimiento de los modelos puede verse condicionado por cambios en los patrones de consumo o por la disponibilidad de nuevos datos representativos.


\section{Objetivos alcanzados}

Los objetivos propuestos al principio del trabajo han sido cumplidos con éxito. En primer lugar, se lleva a cabo un análisis comparativo de varios algoritmos de aprendizaje automático que se utilizan para detectar el fraude eléctrico separando entre técnicas de aprendizaje \emph{offline} y \emph{online}. Para esto, se utilizan métricas de evaluación para implementar y evaluar los modelos, lo que facilita la identificación de las técnicas más efectivas en cada categoría. Además, se realiza el análisis y preprocesamiento de conjuntos de datos ayudando a ajustar la información para entrenar y evaluar los modelos implementados.

El desarrollo de un prototipo de software funcional que pueda incorporar los modelos elegidos dentro de un ambiente web interactivo es otro de los logros obtenidos. También, se incluyen procedimientos de autenticación y administración de usuarios, así como el almacenamiento de datos del sistema en una base de datos, lo que posibilita un entorno estructurado y ordenado para su uso.

En último lugar, el trabajo ha hecho posible corroborar la integración práctica de métodos de aprendizaje automático en una aplicación integral enfocada en identificar fraude eléctrico, logrando las metas que se habían establecido al principio a nivel de experimentación y funcional.


\section{Lecciones aprendidas}

El desarrollo de este trabajo me ha ayudado a adquirir conocimientos tanto en el área del aprendizaje automatizado como en la creación de sistemas informáticos complejos. Casi no contaba con experiencia previa en el funcionamiento interno de muchos, por no decir de casi todos, de los algoritmos de aprendizaje automático empleados al principio del proyecto. Así que, una parte importante del trabajo fue estudiar sus bases teóricas, entender cómo funcionan y analizar su desempeño en diferentes situaciones de detección de fraude eléctrico. Durante el desarrollo del proyecto he aprendido sobre la validación, evaluación y entrenamiento de modelos de clasificación, además de sobre el manejo y preprocesamiento de datos reales. Puedo decir, que hacer este trabajo me permitió entender las diferencias entre los métodos de aprendizaje \emph{online} y \emph{offline}, evaluando lo positivo y lo negativo de cada uno.

Desde la perspectiva del desarrollo de \emph{software}, el proyecto ha sido una muy buena experiencia práctica en la creación y puesta en marcha de una arquitectura web fundamentada en \emph{FastAPI}. Además, se han adquirido conocimientos sobre la creación de \emph{APIs REST}, la identificación a través de \emph{tokens JWT}, la integración con bases de datos \emph{MySQL} y el manejo de usuarios en una aplicación web. Además, he aprendido aspectos sobre la organización de proyectos \emph{software}, gestión de dependencias, despliegue de aplicaciones y configuración de entornos de ejecución reproducibles. Todo ello me permite tener una visión más completa del proceso de desarrollo de sistemas basados en aprendizaje automático, desde la fase experimental hasta su integración dentro de un entorno \emph{software} funcional.


\section{Trabajo futuro}

El sistema desarrollado puede ampliarse en distintas líneas orientadas tanto a la mejora de los modelos de aprendizaje automático como a la evolución del prototipo software implementado. Podrían incorporarse nuevos modelos basados en técnicas de aprendizaje profundo diferentes y con mejor rendimiento que el Perceptrón y el MLP. Así, se podría estudiar su comportamiento sobre curvas de consumo eléctrico más complejas. Además, sería interesante integrar mecanismos de actualización automática de modelos y reentrenamiento periódico utilizando nuevos datos reales de consumo. Esto permitiría mejorar la capacidad de adaptación del sistema ante cambios en los patrones de comportamiento eléctrico y posibles nuevas estrategias de fraude.

Otra posible línea futura sería  sustituir la simulación actual de flujo en tiempo real por una integración directa con dispositivos inteligentes de medición o plataformas \emph{IoT} \cite{IoT}, permitiendo evaluar el sistema en escenarios reales de monitorización continua. Desde el punto de vista software, podrían añadirse nuevas funcionalidades orientadas a la administración avanzada del sistema, generación automática de informes, auditoría de predicciones o paneles interactivos más completos para el análisis visual de resultados.

El procesamiento de grandes archivos puede incrementar significativamente el tiempo de respuesta del sistema, debido a que la carga, predicción y almacenamiento se realizan de forma síncrona dentro de la API. Como mejora futura, podrían incorporarse mecanismos de procesamiento asíncrono o colas de tareas. Aunque el sistema ha sido desplegado sobre infraestructura cloud mediante \emph{AWS}, como trabajo futuro podrían incorporarse mecanismos adicionales de escalabilidad, monitorización y procesamiento distribuido orientados a entornos de producción con mayores volúmenes de datos.
...
El funcionamiento de la interfaz web desarrollada para el prototipo de detección de fraude eléctrico es descrito en este capítulo. Mediante esta interfaz, el operador puede autenticarse en el sistema, importar archivos que contengan curvas de consumo, realizar predicciones tanto históricas como en tiempo real y revisar estadísticas y gráficos creados por los modelos de aprendizaje automático que están integrados en la aplicación. Las secciones siguientes muestran el procedimiento para utilizar cada funcionalidad que esté disponible, utilizando ejemplos y capturas de la aplicación web.


\section{Acceso al sistema}

Para acceder a la web, el usuario tiene que abrir el navegador web e ir a la URL donde está desplegada la aplicación. La dirección es x. Cuando se cargue el sistema, la pantalla de inicio de sesión aparecerá automáticamente. La interfaz de inicio de sesión que ha sido implementada en el sistema se puede observar en la Figura~\ref{fig:login}.
...
Para acceder, se necesita un nombre de usuario y una contraseña que hayan sido registrados en el sistema con anterioridad. Los pasos que se deben seguir para iniciar sesión son los siguientes:

\begin{enumerate}
    \item Introducir el nombre de usuario en el campo correspondiente.
    \item Introducir la contraseña de acceso en el campo correspondiente.    
    \item Hacer click en \emph{Entrar}.
\end{enumerate}

El sistema redirige automáticamente al menú principal de la aplicación si se introducen las credenciales adecuadas. Desde allí, el operador tiene acceso a las diferentes funciones disponibles. La pantalla de inicio del sistema se muestra en la Figura~\ref{fig:home}.

Si se ingresan datos erróneos, la aplicación mostrará un mensaje de error que indica que las credenciales no son válidas. La Figura~\ref{fig:login_error} muestra la interfaz de inicio de sesión cuando se introduce mal las credenciales del usuario. 

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/login_error.png}
\caption{Pantalla de inicio de sesión con error al introducir credenciales.}
\label{fig:login_error}
\end{figure}

\noindent Si se ingresan datos erróneos, hay que seguir estos pasos:

\begin{enumerate}
    \item Hacer click en \emph{Aceptar}.
    \item Introducir los datos de acceso correctos en cada campo correspondiente.
    \item Hacer click en \emph{Entrar}.
\end{enumerate}
...
Cuando el operador se autentica de forma correcta, entra al inicio de la aplicación web, desde donde tiene acceso a las diferentes funciones que se han incorporado. La interfaz web se organiza con un menú lateral, situado a la izquierda de la pantalla, que facilita el movimiento entre las distintas secciones del sistema. La pantalla de inicio se muestra en la Figura~\ref{fig:home}.
...
Las funciones que se encuentran disponibles son: la importación de archivos, las predicciones en tiempo real y las históricas, así como la consulta de estadísticas producidas por los modelos de aprendizaje automático incorporados a la aplicación. El funcionamiento de cada módulo del sistema se detalla a continuación en las subsecciones.


\subsection{Importar archivos}

La función de importación suma la incorporación de archivos en formato .csv que albergan curvas de consumo eléctrico, lo que permite realizar análisis estadísticos y predicciones a continuación. Esta operación se lleva a cabo desde la página principal del sistema, la cual se puede acceder después de haber iniciado sesión de manera correcta. Para poder llevar a cabo todas las demás funcionalidades, es necesario importar al menos un archivo. La Figura~\ref{fig:importar} muestra la opción de importar un archivo.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/importar.png}
\caption{Importar un archivo.}
\label{fig:importar}
\end{figure}

La selección para importar archivos se ubica en la parte superior de la interfaz. El operador tiene que elegir la opción de selección de archivo y elegir un archivo válido guardado en el equipo. Después, para transferir el archivo al sistema, se debe hacer click en la opción de \emph{Importar}. Las Figuras~\ref{fig:seleccionar} y~\ref{fig:seleccionado} enseñan el proceso de selección de un archivo para importarlo.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/seleccionar.png}
\caption{Seleccionar un archivo.}
\label{fig:seleccionar}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/seleccionado.png}
\caption{Archivo seleccionado.}
\label{fig:seleccionado}
\end{figure}

Cuando se ha terminado de importar, el archivo queda guardado y accesible para las demás funciones del sistema. Después de una carga exitosa, el sistema presenta un mensaje de confirmación que señala que el archivo ha sido procesado de manera satisfactoria. Además, en los listados que emplean las secciones de predicción histórica y estadísticas, el archivo se incluirá de manera automática. La Figura~\ref{fig:importado} muestra como la subida del archivo se realizó correctamente.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/importado.png}
\caption{Archivo importado.}
\label{fig:importado}
\end{figure}

\noindent En definitiva, los pasos a seguir para importar un archivo correctamente son:
\begin{enumerate}
    \item Hacer click en \emph{Elegir archivo}.
    \item Seleccionar, dentro del selector de archivos, haciendo click en un archivo válido.
    \item Hacer click en \emph{Abrir}.
    \item Hacer click en \emph{Importar archivo CSV}.
\end{enumerate}

Si el archivo tiene un formato erróneo o si ocurre un fallo durante la carga, se indicará al operador sobre la incidencia ocurrida mediante un mensaje de error del sistema. La Figura~\ref{fig:importar_error} muestra como se produce un error al importar un archivo no válido.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/importar_error.png}
\caption{Error al importar un archivo.}
\label{fig:importar_error}
\end{figure}


\subsection{Consulta de estadísticas}

El sistema en la pantalla de inicio incorpora la consulta de estadísticas para el análisis global de los archivos importados por el operador. Desde el inicio, el usuario puede seleccionar cualquiera de los archivos previamente importados en el sistema. Una vez seleccionado el archivo, se genera automáticamente un conjunto de representaciones gráficas asociadas a las curvas de consumo contenidas en el archivo elegido permitiendo al operador identificar rápidamente comportamientos anómalos o normales.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/seleccionar_curva.png}
\caption{Seleccionar curva.}
\label{fig:seleccionar_archivo}
\end{figure}

Las estadísticas y gráficas que se muestran en la figuras~\ref{fig:grafica1} y~\ref{fig:grafica2} calculan la distribución de fraude, el top de consumo medio de todas las curvas, el top de curvas con mayor probabilidad de fraude y el rango de probabilidades de fraude.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/grafica1.png}
\caption{Gráficas mostradas de un archivo.}
\label{fig:grafica1}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/grafica2.png}
\caption{Otras gráficas mostradas de un archivo.}
\label{fig:grafica2}
\end{figure}


\subsection{Predicción histórica}

La función de predicción histórica permite analizar una curva de consumo completa. Para realizar una predicción histórica, el usuario debe acceder al apartado correspondiente desde el menú principal haciendo click en \emph{Predicción histórica}. A continuación, deberá seleccionar un archivo previamente importado y posteriormente una de las curvas disponibles dentro de dicho archivo.

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{imagenes/menuH.png}
\caption{Menú de la izquierda seleccionando predicción histórica.}
\label{fig:menuH}
\end{figure}
...
\centering
\includegraphics[width=1\textwidth]{imagenes/seleccionar_curva.png}
\caption{Seleccionar curva de un archivo en predicción histórica.}
\label{fig:seleccionar_curva}
\end{figure}

Una vez seleccionada la curva, el sistema muestra la gráfica de consumo junto al resultado y probabilidad de fraude. En la figura~\ref{fig:historica} puede verse la gráfica, donde el eje horizontal representa la evolución temporal completa de la curva, incluyendo la fecha y hora de cada medición, mientras que el eje vertical muestra los valores de consumo eléctrico registrados.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/historica.png}
\caption{Predicción \emph{offline}.}
\label{fig:historica}
\end{figure}


\subsection{Predicción en tiempo real}

La función de predicción en tiempo real permite simular el análisis progresivo de una curva de consumo eléctrico, reproduciendo el comportamiento de un sistema de monitorización continua. Para utilizar esta funcionalidad, el usuario debe acceder al apartado de predicción en tiempo real desde el menú principal haciendo click en \emph{Predicción tiempo real}. Posteriormente, deberá seleccionar un archivo previamente importado y una curva de consumo asociada.

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{imagenes/menuT.png}
\caption{Menú de la izquierda seleccionando predicción en tiempo real.}
\label{fig:menuT}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/seleccionar_curva2.png}
\caption{Seleccionar curva de un archivo en predicción en tiempo real.}
\label{fig:seleccionar_curva2}
\end{figure}

Una vez iniciada la simulación, en las figuras~\ref{fig:real} y~\ref{fig:real2} el sistema representa gráficamente los valores de consumo de manera secuencial, incorporando nuevos puntos de consumo de forma periódica para emular la llegada de datos en tiempo real. Además, presenta al lado derecho el resultado y probabilidad de fraude en cada punto.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/real.png}
\caption{Predicción \emph{online}.}
\label{fig:real}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagenes/real2.png}
\caption{Predicción \emph{online} más adelante en el tiempo.}
\label{fig:real2}
\end{figure}

En esta representación, el eje horizontal muestra los instantes temporales simulados de adquisición de datos, mientras que el eje vertical representa los valores de consumo eléctrico.


\subsection{Cierre de sesión}

El sistema incorpora un mecanismo de cierre de sesión para garantizar la seguridad y el control de acceso al sistema. El usuario puede finalizar su sesión en cualquier momento haciendo click en \emph{Cerrar sesión} situado abajo a la izquierda en cualquier pantalla en la que se encuentre. En la figura~\ref{fig:cerrar} se muestra un ejemplo de la opción de cerrar sesión.

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{imagenes/cerrar.png}
\caption{Cerrar sesión.}
\label{fig:cerrar}
\end{figure}

Al realizar esta acción, el sistema elimina el token de autenticación almacenado localmente en el navegador y redirige automáticamente al usuario a la pantalla de inicio de sesión. De esta forma, se evita el acceso no autorizado a las funciones de la plataforma una vez finalizada la sesión activa del operador.
...
\bibitem{articuloBase} k -means, one-class SVM, Birch and Gaussian mixture model
Performance Analysis of Electricity Theft Detection for the Smart Grid.
...
\bibitem{articulo7} Gaussian
Rule-based classification of energy theft and anomalies in consumers load demand profile.
Disponible en: \url{https://ietresearch.onlinelibrary.wiley.com/doi/full/10.1049/iet-stg.2019.0081}

\bibitem{articulo8} k-means and Birch 
A Novel Combined Data-Driven Approach for Electricity Theft Detection.
Disponible en: \url{https://ieeexplore.ieee.org/abstract/document/8481475?casa_token=ueAtCt5RbscAAAAA:SfuRkCLedA36L1gNRxNHruyAvCD6ngro6UHoadLIj7WgRyPH4NwikpUbkrC1NGYhiXLKZ68_ihE}

\bibitem{articulo9} a fuzzy c-means (FCM) clustering
A Tunable Fraud Detection System for Advanced Metering Infrastructure Using Short-Lived Patterns.
Disponible en: \url{https://ieeexplore.ieee.org/abstract/document/8039510?casa_token=9Cc6g40zRWoAAAAA:fdOvTeYfgiHg6hPOFHhu4f1Ty34kiKbd2cfZtvuOUBdOFYnDPC7F7yKE3OzbVatT70bRngGIgWw}

\bibitem{articulo10} Gaussian mixture model (GMM) y  CVMM
C-Vine Copula Mixture Model for Clustering of Residential Electrical Load Pattern Data.
Disponible en: \url{https://ieeexplore.ieee.org/abstract/document/7579208?casa_token=S-ur8QWcqKoAAAAA:C_KWNOxeEk_lrc2oxLcnFMoaavnxlNWIOxuk8RJ-Bv2M9UDr4THjmKXa5Aml0EVFUehTk2gPhEY}
...
\bibitem{IoT}
Plataformas IoT.
Disponible en: \url{https://www.cognizant.com/es/es/glossary/internet-of-things-platform}
